...
public class WorldController {
    ...
    @PostMapping("/grid/{id}")
    public RestResponse<List<Cell>> getGridLocation(@PathVariable("id") int id, @RequestBody Location pos){
        RestResponse<List<Cell>> response = null;
        try {
            List<Cell> grid = worldService.findAllCell(id, pos.getPosX(), pos.getPosY());
            response = new RestResponse<>(grid);
